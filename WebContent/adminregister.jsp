		...
		if(request.getParameter("submit")!=null){			
			if(!request.getParameter("adminname").isEmpty() && !request.getParameter("canteenname").isEmpty() && 
				...
				if(request.getParameter("phone").length()!=10){
					out.print("Length of phone number should be 10 digit");
					error = true;
				}
				...
				if(!(firstChar >=90 &&firstChar<=122)){
					out.println("First character must be small");					
					error = true;
				}
				...
				for(int i=0;i<email.length();i++)
					if(email.charAt(i)=='@')
						count++;
				if(count!=1){
					out.println("occurrence of @ is not properly maintained");
					error = true;					
				}
				...
				for(int i=0;i<email.length();i++){
					...
					if(!((unicode>=64 && unicode <=90)||(unicode >=97 && unicode<=122)||(unicode==95)||(unicode==46)||(unicode>=48&& unicode<=57))){
						out.println("Special symbol is not allowed in the email address");
						error = true;
					}
				}
				...
				out.print("All the fields are required");
			}  
		}
